...
```text
                 ...
                 | 串口控制与应用验证      |   | UUU/USB 烧录            |
                 | reboot/U-Boot/login    |   | SDP/Fastboot/lsusb     |
                 +-----------+------------+   +------------+-----------+
                             |                             |
                             +-------------+---------------+
                                           |
...
- `烧录控制串口`
  - 从 Linux shell 发送 `reboot`
  - 抢占 `U-Boot`
  - 执行 `fastboot 0` 或 `bmode usb`
- `应用调试串口`
  - 观察启动日志
  - 默认按 `root` 无密码场景处理 `login:`
  - 看到 shell 后直发 `which hello; hello` 等验证命令
- 当前实现要求：
  - 两类动作都在 `linux_serial_agent` 内完成
  - 串口日志必须可见，用户能直接看到脚本发送了什么、设备回了什么
  - `flash_usb_shell` 只负责 USB 传输，不再承担串口状态机
...
make -j$(nproc)
        |
        v
确认 output/images/sdcard.img 与目标程序已生成
        |
        v
串口进入 Linux shell
        |
...
脚本发送 reboot -> 抢 U-Boot -> fastboot 0
        |
        v
uuu 只执行 flash -raw2sparse all sdcard.img
        |
        v
系统重启 -> 串口直发 which hello; hello
        |
        v
收集返回码与日志，给出验证结论
```
...
- 现阶段建议优先使用串口脚本内置的 `root` 默认登录 + 直发命令验证，避免等待 `Password:` 超时。
